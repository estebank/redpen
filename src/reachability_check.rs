...
pub struct ReachabilityCheck<'tcx, C: Config> {
    tcx: TyCtxt<'tcx>,
...
impl<'tcx, C: Config> ReachabilityCheck<'tcx, C> {
    ...
    pub fn check_attrs(&mut self, def_id: DefId) -> bool {
        if let Some(did) = def_id.as_local()
            ...
            && let Some(_) = self
                ...
                .filter(|attr| self.config.is_disabled_scope(attr))
                .next()
        {
...
impl<'tcx, C: Config> Visitor<'tcx> for ReachabilityCheck<'tcx, C> {
...
impl<'tcx, C: Config> LateLintPass<'tcx> for ReachabilityCheck<'tcx, C>
where
    ReachabilityCheck<'tcx, C>: LintPass,
{
    fn check_crate(&mut self, _cx: &LateContext<'tcx>) {
        ...
        for (accessee, accessed) in &backward {
            ...
            if self.config.is_relevant_leaf(name.as_str()) {
            ...
            for item in accessees {
                ...
                if affected.contains(&accessee) {
                    ...
                    let is_generic = accessor
                        .args
                        .non_erasable_generics(self.tcx, accessee.def_id())
                        .next()
                        .is_some();
                    ...
                    self.tcx.struct_span_lint_hir(
                        ...
                        format!(
                            "calling `{}` {}{}",
                            accessee_path,
                            self.config.affected_description(),
                            generic_note
                        ),
                        |diag| {
                            diag.span_label(
                                item.span,
                                format!("this call {}", self.config.affected_description()),
                            );
        ...
        // Handle reachability through trait objects.
